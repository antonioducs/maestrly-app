...
test('Gitleaks uses only constrained current-tree exceptions', () => {
  ...
  assert.equal((source.match(/targetRules = \["generic-api-key"\]/g) ?? []).length, 4)
  assert.equal((source.match(/condition = "AND"/g) ?? []).length, 4)
  assert.equal((source.match(/regexTarget = "line"/g) ?? []).length, 4)
