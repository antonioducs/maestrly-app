...
2. Under **Projects on this computer**, select the Kanban project, then the folder on this computer. Use **Choose folder** for an existing checkout (a folder that is not a Git repository yet is initialized automatically), or **Clone <repository>** when the Kanban repository has a clone URL — the desktop clones it with your local Git credentials and selects it. Choose **Link project**. Without a Kanban repository binding, column agents run in an empty workspace.
