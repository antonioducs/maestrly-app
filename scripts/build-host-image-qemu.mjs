...
import { cp, mkdtemp, mkdir, readFile, rm, writeFile, open, chmod, rename, stat, lstat } from 'node:fs/promises'
import path from 'node:path'
import { randomUUID } from 'node:crypto'
import { validateBuildConfig, verifyInput, sha256, run } from './host-build-utils.mjs'

export function imageBuildUserData(config, instanceId) {
  ...
  return `#cloud-config\nusers: []\ndisable_root: true\nssh_pwauth: false\npackage_update: true\npackage_upgrade: false\npackages:\n  - [qemu-guest-agent, '${config.guestAgentVersion}']\nruncmd:\n  - [sh, -c, ${JSON.stringify(cleanup)}]\n`
}

export async function buildQemuImage(config, root) {
  ...
  const userData = imageBuildUserData(config, instanceId)
  const output = path.join(root, 'dist', `ubuntu-24.04-${config.releaseDate}-arm64.qcow2`)
  try {
    await lstat(output)
  ...
  const temporary = await mkdtemp('/private/tmp/mhi-')
  ...
  try {
    ...
    await writeFile(
      ...
      `instance-id: ${instanceId}\nlocal-hostname: maestrly-image-build\n`
    )
    await writeFile(
    ...
    const args = [
      ...
      '-blockdev',
      JSON.stringify({ driver: 'file', filename: staging, 'node-name': 'image-file' }),
      '-blockdev',
      JSON.stringify({ driver: 'qcow2', file: 'image-file', 'node-name': 'image' }),
      '-device',
      ...
      '-blockdev',
      JSON.stringify({ driver: 'raw', file: 'seed-file', 'node-name': 'seed', 'read-only': true }),
      '-device',
    ...
    const manifest = {
      version: 1,
      id: `ubuntu-24.04-${config.releaseDate}-arm64`,
      architecture: 'arm64',
