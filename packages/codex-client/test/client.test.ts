...
lines.on('line', (line) => {
  ...
  switch (message.method) {
    ...
    case 'test/error':
      ...
      break
    case 'test/exactLine':
      const prefix = JSON.stringify({ method: 'fixture/notification', params: '' })
      const line = JSON.stringify({
        method: 'fixture/notification',
        params: 'é' + 'x'.repeat(1024 - Buffer.byteLength(prefix) - 2),
      })
    ...
    case 'account/login/cancel':
      success(message.id, message.params)
      break
    case 'model/list':
      success(message.id, { models: [], ...message.params })
      break
    case 'test/hang':
      break
    case 'test/protocol':
...
describe('CodexAppServerClient', () => {
  ...
  it('drops oversized lines across chunks and recovers at the next newline', async () => {
    const client = await connect({ maxLineBytes: 1024 })
    const errors: CodexAppServerProtocolError[] = []
    client.onProtocolError((error) => errors.push(error))
    await expect(client.request('test/oversized')).resolves.toEqual({ recovered: true })
    expect(errors).toHaveLength(1)
    expect(errors[0]).toBeInstanceOf(CodexAppServerProtocolError)
    expect(errors[0].message).toBe('Codex app-server line exceeds limit')
    expect(errors[0].line).toBeUndefined()
    expect(client.state).toBe('ready')
    await expect(client.request('test/handshake')).resolves.toEqual({ initialized: true })
  })

  it('accepts exact byte limits with split UTF-8 and CRLF', async () => {
    const client = await connect({ maxLineBytes: 1024 })
    const errors: unknown[] = []
    const notifications: unknown[] = []
    client.onProtocolError((error) => errors.push(error))
    client.onNotification((notification) => notifications.push(notification))
    await expect(client.request('test/exactLine')).resolves.toEqual({})
    expect(errors).toEqual([])
    expect(notifications).toEqual([{ method: 'fixture/notification', params: expect.stringMatching(/^éx+$/) }])
  })

  it('bounds pending requests and releases capacity after cancellation', async () => {
    const client = await connect({ maxPendingRequests: 2 })
    const abort = new AbortController()
    const pending = Promise.allSettled([
      client.request('test/hang', {}, { signal: abort.signal }),
      client.request('test/hang', {}, { signal: abort.signal }),
    ])
    await expect(client.request('test/hang')).rejects.toBeInstanceOf(CodexAppServerLimitError)
    abort.abort()
    expect((await pending).map((result) => result.status)).toEqual(['rejected', 'rejected'])
    await expect(client.request('test/handshake')).resolves.toEqual({ initialized: true })
  })

  it('only inherits the minimal environment and explicit overrides', async () => {
    process.env.MAESTRLY_CODEX_ENV_TEST = 'secret'
    process.env.OPENAI_BASE_URL = 'https://attacker.invalid/v1'
    const client = await connect({
  ...
  it('supports login cancellation and model listing', async () => {
    const client = await connect()
    await expect(client.cancelAccountLogin({ loginId: 'login-1' })).resolves.toEqual({ loginId: 'login-1' })
    await expect(client.listModels()).resolves.toEqual({ models: [] })
    await expect(client.listModels({ cursor: 'next' })).resolves.toEqual({ models: [], cursor: 'next' })
  })
...
const fixture = path.resolve('test/fixtures/app-server.mjs')
