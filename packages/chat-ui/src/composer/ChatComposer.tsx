...
export function ChatComposer({
  ...
  const filtered = useMemo(() => {
    ...
    return commands.filter((c) => c.name.toLowerCase().includes(q) || (c.description?.toLowerCase().includes(q) ?? false))
  }, [slashQuery, commands])
  ...
  return (
    ...
    >
      {paletteOpen && (
        <div role="listbox" aria-label={labels.composer.commands} className="absolute bottom-full left-3 z-50 mb-1 max-h-72 w-96 overflow-auto rounded-lg border border-white/[0.1] bg-[#161618] p-1 shadow-2xl">
          {filtered.map((command, index) => (
            <button
              ...
              className={cn('flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left', index === commandIndex ? 'bg-white/[0.06]' : 'hover:bg-white/[0.04]')}
            >
              <span className="shrink-0 font-mono text-[13px] text-foreground">
                /{command.name}
                {command.argumentHint && <span className="ml-1 text-muted-foreground/70">{command.argumentHint}</span>}
              </span>
              {command.description && <span className="min-w-0 flex-1 truncate text-[12px] text-muted-foreground">{command.description}</span>}
              {command.kind && <span className="ml-auto shrink-0 rounded text-[10px] uppercase tracking-wide text-muted-foreground/60">{command.kind}</span>}
            </button>
          ))}
        </div>
      )}
      {attachments.length > 0 && (
        <div className="composer-attachments mb-2 flex flex-wrap gap-2" aria-label={labels.composer.attachments}>
          {attachments.map((file) => (
            <span key={file.id} className="attachment-chip inline-flex items-center gap-1.5 rounded-md border border-white/[0.08] bg-white/[0.04] px-2 py-1 text-[12px] text-foreground">
              ...
              {onRemoveAttachment && (
                <Button type="button" aria-label={`${labels.composer.removeAttachment} ${file.name}`} disabled={streaming || busy} onClick={() => onRemoveAttachment(file.id)}>
      ...
      <div className="actions mt-1.5 flex items-center justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {micSlot}
          {streaming ? (
            <Button type="button" disabled={disabled || stopping || busy} onClick={onStop}>
              <Square size={12} aria-hidden="true" />
              {stopping ? labels.composer.stopping : labels.composer.stop}
            </Button>
          ) : (
            <Button className="send-button" type="submit" aria-label={labels.composer.send} title={labels.composer.send} disabled={!canSend}>
              <ArrowUp size={18} aria-hidden="true" />
            </Button>
          )}
        </div>
      </div>
      {metaSlot && <div className="composer-meta mt-1.5 flex w-full items-center justify-between gap-2 px-1 text-[11px] text-muted-foreground">{metaSlot}</div>}
