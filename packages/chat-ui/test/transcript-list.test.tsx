...
const user: TranscriptMessageLike = { id: 'u1', role: 'user', createdAt: at, parts: [{ type: 'text', id: 'u1:text', text: 'list files' }], streaming: false }
...
describe('TranscriptList', () => {
  it('renders a running tool card, folded reasoning and markdown text for a streaming card', () => {
    const html = renderToStaticMarkup(<Wrap><TranscriptList messages={[user, card]} /></Wrap>)
  ...
  it('formats the time of each message in the injected locale', () => {
    const en = renderToStaticMarkup(<Wrap><TranscriptList messages={[user]} /></Wrap>)
    const pt = renderToStaticMarkup(<Wrap locale="pt-BR"><TranscriptList messages={[user]} /></Wrap>)
  ...
  it('hands files and system notices to the application through slots', () => {
    const system: TranscriptMessageLike = { id: 's', role: 'system', createdAt: at, parts: [{ type: 'text', id: 's:t', text: 'internal' }], streaming: false }
    const withFile: TranscriptMessageLike = { ...card, streaming: false, parts: [{ type: 'file', id: 'f', path: 'a.md', name: 'a.md', size: 1 }] }
    const html = renderToStaticMarkup(
      <Wrap>
        <TranscriptList messages={[system, withFile]} slots={{ system: () => <em>notice</em>, file: (part) => <b>{part.name}!</b> }} />
      </Wrap>
    )
