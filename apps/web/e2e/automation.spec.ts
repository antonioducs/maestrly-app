...
test('configures independent column agents with effort, fast, prompt preview and history',async({page},info)=>{
 ...
 await dialog.getByRole('radiogroup',{name:L('Execution mode'),exact:true}).getByRole('radio',{name:'Maestro',exact:true}).click()
 ...
 // Layout stays put while selecting: the preview is inline and every field keeps its slot.
 await dialog.getByRole('button',{name:L('Show rendered prompt'),exact:true}).click()
 const preview=dialog.locator('.af-preview-body')
 await expect(preview).toContainText('Build My task: Task description')
 await expect(preview).toContainText('Card ID: card')
 ...
 // Selections must never move the sections: fields are disabled with a hint instead of unmounted.
 const sectionTops=()=>dialog.locator('.af-body').evaluate(body=>[...body.querySelectorAll('.af-sec')].map(s=>Math.round(s.getBoundingClientRect().top+body.scrollTop)))
 const before=await sectionTops()
 await dialog.getByRole('combobox',{name:L('Execution destination'),exact:true}).click()
 await page.getByRole('option',{name:L('Specific runner'),exact:true}).click()
 ...
 await dialog.getByRole('radiogroup',{name:L('Execution mode'),exact:true}).getByRole('radio',{name:L('Standard'),exact:true}).click()
 expect(await sectionTops()).toEqual(before)
 await dialog.getByRole('combobox',{name:L('Execution destination'),exact:true}).click()
 await page.getByRole('option',{name:L('Compatible runner pool'),exact:true}).click()
 await dialog.locator('.af-body').evaluate(el=>{el.scrollTop=0})
 ...
 await expect(dialog.getByRole('checkbox',{name:L('Fast mode'),exact:true})).toBeDisabled()
 await dialog.getByRole('checkbox',{name:L('Agent enabled'),exact:true}).check()
