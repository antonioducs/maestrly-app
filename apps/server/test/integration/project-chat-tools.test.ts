  ...
  try {
    ...
    await inTenantTransaction(f.pool, { ...f.scope, actor: { type: 'human', userId: f.scope.userId } }, async (c) => {
      ...
      await c.query(
        ...
        [f.scope.organizationId, f.scope.projectId, userId]
      )
    })
    const scope = { ...f.scope, userId },
      session = await createSession(f.pool, scope, { ...f.session, title: 'Contributor chat' })
    await chatTransaction(f.pool, scope, true, async (c) =>
      enqueueMessage(c, await ownedSession(c, scope, session.id, true), 'Read the board', crypto.randomUUID())
    )
    const claim = (await claimChat(f.pool, f.identity))!
    expect(claim.session.ownerUserId).toBe(userId)
    await expect(
      executeChatTool(f.pool, f.scope.organizationId, claim.token, 'board_list_boards', {}, 'allowed')
    ).resolves.toBeDefined()
    await inTenantTransaction(f.pool, { ...f.scope, actor: { type: 'human', userId: f.scope.userId } }, (c) =>
      c.query("update project_members set role='viewer' where project_id=$1 and user_id=$2", [
        f.scope.projectId,
        userId,
      ])
    )
    await expect(
      executeChatTool(f.pool, f.scope.organizationId, claim.token, 'board_list_boards', {}, 'revoked')
