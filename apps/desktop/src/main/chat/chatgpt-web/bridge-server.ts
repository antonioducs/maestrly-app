...
import type { ConversationFileScope } from '../../conversation-file-scope'
...
export interface BridgeOptions {
  ...
  fileScope?: ConversationFileScope
  kanban?: {
    context(): unknown
...
export function createChatGptWebBridge(options: BridgeOptions) {
  ...
  function investigationReport(disclosure: InvestigationDisclosure): string {
    ...
    const lines = [
      ...
      '',
      options.conversationScope === 'standalone'
        ? `- Conversation context loaded: ${contextLoaded ? 'yes' : 'no'}; no repository attached.`
    ...
    if (options.conversationScope === 'standalone') {
      lines.push('', readFiles.size || externalEvidence.size
        ? 'Analysis based on the recorded file and tool evidence; no repository was inspected.'
        : 'Self-contained analysis: no file or external-source reads were recorded.')
      if (externalEvidence.size) lines.push('', '### External tools actually used',
        ...limited(externalEvidence).map(item => `- ${reportItem(item)}`))
    }
    lines.push(
      '',
  ...
  const canonicalPathInside = async (
    ...
    if (options.fileScope) return options.fileScope.resolveBridgePath(rel)
  ...
  async function toolSendToMaestrly(args: Record<string, unknown>) {
    ...
    if (options.conversationScope !== 'standalone' && searches.length === 0) {
      return ERR('Insufficient investigation: use grep or glob to locate the relevant scope before publishing.')
    }
    if (options.conversationScope !== 'standalone' && readFiles.size === 0) {
      return ERR(
  ...
  async function buildContext(signal: AbortSignal, pendingStages: Set<string>): Promise<BridgeToolResult> {
    ...
    if (options.conversationScope === 'standalone') {
      ...
      return TEXT(['# Standalone conversation context',
        'Files are in the private conversation directory. No repository is attached.',
        'Use glob, grep and read_file when file evidence is relevant. Git, GitHub, project memory and boards are unavailable.',
        ...(skills.length ? ['## Available configured skills', ...skills.map(skill => `- ${skill.name}: ${skill.description}`)] : []),
      ].join('\n'))
    }
   ...
   */
  const requiresProject = (name: string): boolean => [
    'git_diff', 'git_read', 'gh_read', 'run_check', 'start_review_loop', 'submit_review_fix',
    'wait_review_fix', 'finish_review_loop', 'discover_frontend_previews', 'start_project_environment',
    'wait_project_environment', 'cancel_project_environment', 'search_project_memory', 'read_project_memory_source',
    'get_linked_kanban',
  ].includes(name) || name.startsWith('board_') || name.startsWith('kanban_')

  function listTools() {
    return TOOLS.filter(tool => options.conversationScope !== 'standalone' || !requiresProject(tool.name))
      .map(({ run: _run, ...tool }) => withSessionKey(tool))
  }
  ...
  async function callTool(name: string, args: Record<string, unknown>): Promise<BridgeToolResult> {
    if (ended) return ERR(SESSION_ENDED_MESSAGE)
    if (options.conversationScope === 'standalone' && requiresProject(name))
      return ERR('no_repository: This operation requires a project conversation.')
    ...
    try {
      ...
      if (ok && ['call_mcp_read_tool', 'browser_read_text', 'browser_snapshot', 'browser_screenshot'].includes(name)) {
        externalEvidence.add([name, args.server_id, args.tool_name].filter(value => typeof value === 'string').join(': '))
      }
