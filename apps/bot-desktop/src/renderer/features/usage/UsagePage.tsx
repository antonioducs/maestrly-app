import { UsagePanel, fmtCost, fmtTokens, useChatUi } from '@maestrly/chat-ui'
import { useT } from '../../i18n'
import { useUsage } from './useUsage'

/** Usage of every bot on the connected Host: the shared table by model, then a breakdown by bot. */
export function UsagePage({ connected, supported }: { connected: boolean; supported: boolean }) {
  const t = useT()
  ...
  return (
    <section className="settings usage-page">
      <h1>{t('usage')}</h1>
