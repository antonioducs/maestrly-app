...
export function EnvironmentsPage({ hosts, connect, refreshHosts, advanced }: { hosts: HostTarget[]; connect: (target: HostTarget) => Promise<void>; refreshHosts: () => Promise<void>; advanced: () => void }) {
  ...
  return <section className="settings environments-page">
    ...
    {environments.map(environment => <Surface key={environment.vm.id} className="environment-card">
      ...
      {environment.updateAvailable === 'desktop' && <p>{t('desktopUpdateAvailable')}</p>}
      {(['needs-preparation', 'needs-update', 'needs-migration'].includes(environment.status) || environment.updateAvailable === 'desktop') && <Button disabled={busy} onClick={() => { setPreparing(environment); setBackup(false); setRestart(false) }}>{t(environment.status === 'needs-update' || environment.updateAvailable === 'desktop' ? 'updateEnvironment' : environment.status === 'needs-migration' ? 'migrateEnvironment' : 'prepareEnvironment')}</Button>}
