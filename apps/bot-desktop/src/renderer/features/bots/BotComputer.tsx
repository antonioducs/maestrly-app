...
export function BotComputer({ bot }: { bot: Bot }) {
  const t = useT()
  ...
  return <div className="bot-computer-summary"><Monitor size={16} aria-hidden="true" /><div><strong>{t('privateWorkspace')}</strong><p>{value.vm.name}</p><small>{t('sharedComputerExplanation')}</small></div></div>
}
